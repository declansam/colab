...
def scatter_mean(
    ...
    count = scatter_sum(
        preserved_edges, index, dim, None, dim_size
    )  # count for each triple in batch, & for each node, how many edges they appear in
    count[count < 1] = 1
...
def customized_scatter(
    ...
    if reduce == "mul":
        ...
        return scatter_mean(
            src, index, preserved_edges, dim, out, dim_size
...
edge_weight, index, edge_filter, node_dim, dim_size = torch.load(
    "/scratch/rk3570/Graph-Transformer/NBFNet-PyG/misc/aggr.pt"
)
# edge_filter = torch.ones_like(edge_filter)

start = time.time()
for i in range(100):
    mean = customized_scatter(
        edge_weight,
        index,
