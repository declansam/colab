...
if __name__ == "__main__":
    ...
    cfg.model.random_node_features = random.choice(['const', 'normal'])
    if cfg.dataset['class'] == 'synthetic':
        cfg.model.input_dim = random.choice([16, 32, 64])
        cfg.train.num_epoch = random.choice([100, 150, 200])
    else:
        cfg.model.input_dim = random.choice([16, 32])
        cfg.train.num_epoch = random.choice([20])
        
    cfg.model.hidden_dims = [cfg.model.input_dim, cfg.model.input_dim, cfg.model.input_dim]
    cfg.model.aggr = random.choice(['sum', 'mean'])
    cfg.task.sample_neg_edges = random.choice(['yes', 'no'])

    # cfg.optimizer.lr = random.choice([1e-2, 5e-3, 1e-3, 5e-4, 1e-4, 5e-5])
    cfg.optimizer.lr = random.choice([1e-3, 5e-4, 1e-4, 5e-5])
