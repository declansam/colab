...
class GeneralizedRelationalConv(MessagePassing):
    ...
    def __init__(self, input_dim, output_dim, num_relation, query_input_dim, message_func="distmult",
                 aggregate_func="pna", layer_norm=False, activation="relu", dependent=True, use_pyg_propagation=False):
    ...
    def forward(self, input, query, boundary, edge_index, edge_type, size, edge_weight=None, preserved_edges=None, drop_edges=False):
        ...
        output = self.propagate(input=input, relation=relation, boundary=boundary, edge_index=edge_index,
                                edge_type=edge_type, size=size, edge_weight=edge_weight, preserved_edges=preserved_edges, drop_edges=drop_edges)
        return output

    def propagate(self, edge_index, size=None, **kwargs):
        if kwargs["edge_weight"].requires_grad or self.message_func == "rotate" or kwargs["edge_weight"].ndim==2 or self.use_pyg_propagation:
    ...
    def aggregate(self, input, edge_weight, index, dim_size, preserved_edges, drop_edges):
        ...
        if edge_weight.ndim == 2: # we have different edge weight per triple in batch
            self_loop_weights = torch.ones((edge_weight.size(0), dim_size), device=input.device)
            edge_weight = torch.cat([edge_weight, self_loop_weights], dim=-1).unsqueeze(-1)
            if preserved_edges is not None:
                preserved_edges = torch.cat([preserved_edges, self_loop_weights], dim=-1).unsqueeze(-1)
        ...
        if drop_edges: # we will be dropping edges here
            scatter_func = customized_func.customized_scatter
            degree_func = customized_func.customized_degree
            if self.aggregate_func == "pna": # if we are dropping edges, we need to have a customized scatter function
                mean = scatter_func(input * edge_weight, index, preserved_edges=preserved_edges, dim=self.node_dim, dim_size=dim_size, reduce="mean")
                sq_mean = scatter_func(input ** 2 * edge_weight, index, preserved_edges=preserved_edges, dim=self.node_dim, dim_size=dim_size, reduce="mean")
                max = scatter_func(input * edge_weight, index, preserved_edges=preserved_edges, dim=self.node_dim, dim_size=dim_size, reduce="max")
                assert not torch.any(max == float('-inf'))
                min = scatter_func(input * edge_weight, index, preserved_edges=preserved_edges, dim=self.node_dim, dim_size=dim_size, reduce="min")
                assert not torch.any(min == float('inf'))
                std = (sq_mean - mean ** 2).clamp(min=self.eps).sqrt()
                features = torch.cat([mean.unsqueeze(-1), max.unsqueeze(-1), min.unsqueeze(-1), std.unsqueeze(-1)], dim=-1)
                features = features.flatten(-2)
                degree_out = degree_func(index, preserved_edges=preserved_edges, dim=self.node_dim, num_nodes=dim_size).unsqueeze(-1) # customized degree function for dropping edges
                ...
                output = scatter_func(input * edge_weight, index, preserved_edges=preserved_edges, dim=self.node_dim, dim_size=dim_size,
            ...
            degree_func = degree
            if self.aggregate_func == "pna": # if we are dropping edges, we need to have a customized scatter function
                mean = scatter_func(input * edge_weight, index, dim=self.node_dim, dim_size=dim_size, reduce="mean")
                sq_mean = scatter_func(input ** 2 * edge_weight, index, dim=self.node_dim, dim_size=dim_size, reduce="mean")
                max = scatter_func(input * edge_weight, index, dim=self.node_dim, dim_size=dim_size, reduce="max")
                min = scatter_func(input * edge_weight, index, dim=self.node_dim, dim_size=dim_size, reduce="min")
                std = (sq_mean - mean ** 2).clamp(min=self.eps).sqrt()
                features = torch.cat([mean.unsqueeze(-1), max.unsqueeze(-1), min.unsqueeze(-1), std.unsqueeze(-1)], dim=-1)
                features = features.flatten(-2)
                degree_out = degree_func(index, dim_size).unsqueeze(0).unsqueeze(-1) # customized degree function for dropping edges
                scale = degree_out.log()
                scale = scale / scale.mean()
                scales = torch.cat([torch.ones_like(scale), scale, 1 / scale.clamp(min=1e-2)], dim=-1)
                output = (features.unsqueeze(-1) * scales.unsqueeze(-2)).flatten(-2)
            else:
                output = scatter_func(input * edge_weight, index, dim=self.node_dim, dim_size=dim_size,
                                reduce=self.aggregate_func)
