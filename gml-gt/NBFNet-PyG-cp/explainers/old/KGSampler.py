...
class DynamicEgoRelPGExplainer(nn.Module):
    ...
    def forward(self, data, batch, epoch=None):
        ...
        src_node_emb = embeds[data.edge_batch, data.edge_index[0]]
        tgt_node_emb = embeds[data.edge_batch, data.edge_index[1]]

        if self.project_relation:
            R = self.rel_projection(R)

        rel_emb = R[data.edge_type]
        ...
        if self.training:
            ...
            mask, edge_filter = self.transform_mask(data, mask)
    ...
    def transform_mask(self, data, mask):
        ...
        if self.eval_mask_type == 'hard_node_mask_top_ratio':
            ...
            return mask, edge_filter # the location of the mask changes

        elif self.eval_mask_type == 'soft_edge_mask':
            drop_edges = False
            return mask, batch_id, edge_id
