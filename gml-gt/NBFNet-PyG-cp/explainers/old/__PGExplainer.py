...
from explainers._util import index_edge
...
class PGExplainer(BaseExplainer):
    ...
    def __init__(
        ...
        if self.rel_emb:  # z_i, z_r, z_j, z_head
            self.expl_embedding = self.model_to_explain.embedding_size * 4
        else:  # z_i, z_r, z_j
    ...
    def prepare(self, indices=None):
        ...
        if self.type == "graph":
            ...
            self.graphs = self.graphs.to(self.device)
    ...
    def train(self, indices=None):
        ...
        self.model_to_explain.to(self.device)
        ...
        if self.type == "node":
            embeds = self.model_to_explain.embedding(
                self.features, self.graphs
            ).detach()
        ...
        for e in tqdm(range(0, self.epochs)):
            ...
            for n in indices:
                ...
                if self.type == "node":
                    ...
                    feats = self.features
                    graph = ptgeom.utils.k_hop_subgraph(n, self.hops, self.graphs)[1]
                else:
                ...
                mask = self._sample_graph(
                    sampling_weights, t, bias=self.sample_bias
                ).squeeze()
                mask = mask.to(self.device)

                masked_pred = self.model_to_explain(
                    feats, graph, edge_weights=mask
                )  ##FORWARD PASS, POTENTIAL TO USE GPU HERE
                original_pred = self.model_to_explain(
                    feats, graph
                ...
                id_loss = self._loss(
                    masked_pred,
                    torch.argmax(original_pred).unsqueeze(0),
                    mask,
                    self.reg_coefs,
                )
                loss += id_loss
    ...
    def explain(self, index):
        ...
        if self.type == "node":
            # Similar to the original paper we only consider a subgraph for explaining
            graph = ptgeom.utils.k_hop_subgraph(index, self.hops, self.graphs)[1]
            embeds = self.model_to_explain.embedding(
                self.features, self.graphs
            ).detach()
        else:
    ...
    def set_ood_graphs(self, graphs, features):
        """
        Function used for setting ood graphs and features
        :param graphs: the ood graphs used for inductive setting
        :param features: the ood features used for inductive setting
        """
        self.ood_graphs = graphs
        self.ood_features = features

    def inductive_explain(self, index, node_idx=None):
        """
        Function used for explaining graphs in an inductive setting.
        Given the index of a node/graph this method returns its explanation. This only gives sensible results if the prepare method has
        already been called.
        :param index: index of the graph we are using (for node classification, we have k_hop ood subgraphs of the nodes, and for graph classification, we have the ood_graphs)
        :param node_idx: index of the node we are explaining (not used for graph classification)
        :return: explanaiton graph and edge weights
        """
        index = int(index)

        graph = self.ood_graphs[index].clone().detach()  # get the k_hop_ood_subgraph
        if self.type == "graph":
            feats = (
                self.ood_features[index].clone().detach()
            )  # use the ood features for graph
        else:
            feats = (
                self.features
            )  # use original features for node classification (as there is no difference in message propagation to the node as long as there are no connecting edges)
        embeds = self.model_to_explain.embedding(feats, graph).detach()

        # Use explainer mlp to get an explanation
        input_expl = self._create_explainer_input(graph, embeds, node_idx).unsqueeze(
            dim=0
        )
        sampling_weights = self.explainer_model(input_expl)
        mask = self._sample_graph(sampling_weights, training=False).squeeze()
