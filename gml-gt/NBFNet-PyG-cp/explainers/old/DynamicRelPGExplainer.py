...
def batched_k_hop_subgraph(node_idx, num_hops, edge_index, num_nodes):
    ...
    Returns:
        edge_mask: (batch_size, num_edges) indicating whether an edge belongs in the k-hop neighbor
                    for each central edge
    '''
    col, row = edge_index
...
class DynamicRelPGExplainer(nn.Module):
    """
    ...
    :param temp: the temperture parameters dictacting how we sample our random graphs.
        - temp[0] will be the first temperature at the first epoch
        - temp[1] will be the final temperature at the last epoch
    ...
    def __init__(self, 
        ...
        # aggregation w.r.t. to the weights
        self.aggregate_wrt_weights = aggregate_wrt_weights
        ...
        self.eval_on_random = eval_on_random
    ...
    def forward(self, data, batch, epoch=None, save_explanation=False):
        start_time = time.time()
        if not self.joint_training:
            self.model_to_explain.eval()
        # do masking and convert the batch to tail prediction
        data, batch = self.model_to_explain.masking_and_conversion_to_tail_prediction(data, batch)

        h_index, t_index, r_index = batch.unbind(-1) #(batch_size, num_triples)

        # get the k_hop_subgraph of the head for each triple in the batch
        k_hop_time_s = time.time()
        k_hop_subgraphs = batched_k_hop_subgraph(h_index[:, 0], self.hops, data.edge_index, data.num_nodes)
        batch_id, edge_id = k_hop_subgraphs.nonzero().T # get the edge_id for each triple in the batch that are in the k_hop_subgraph of the ehad
        src_node_idx = data.edge_index[0][edge_id] # get the src node idx for each edge
        tgt_node_idx = data.edge_index[1][edge_id] # get the tgt node idx for each edge
        rel_idx = data.edge_type[edge_id] # get the rel idx for each edge
        k_hop_time_t = time.time()
        k_hop_time = k_hop_time_t - k_hop_time_s

        original_pred, embeds, R = self.model_to_explain(data, batch, return_emb=True)
        src_node_emb = embeds[batch_id, src_node_idx]
        tgt_node_emb = embeds[batch_id, tgt_node_idx]
        ...
        if self.training:
            ...
            edge_weight = torch.zeros(k_hop_subgraphs.shape, device=mask.device)
            edge_weight[batch_id, edge_id] = mask
            if self.aggregate_wrt_weights: # aggregation with regards to the weights
                masked_pred = self.model_to_explain(data, batch, edge_weight=edge_weight, preserved_edges=edge_weight, drop_edges=True)
            else:
            ...
            data, mask, drop_edges, batch_id, edge_id = self.transform_mask(data, mask, k_hop_subgraphs, batch_id, edge_id)
            edge_weight = torch.zeros(k_hop_subgraphs.shape, device=mask.device)
            edge_weight[batch_id, edge_id] = mask
            if not self.keep_edge_weight: # the preserved edges has label 1 and otherwise 0
                preserved_edges = edge_weight
            else: # the edge weight will keep its own edge weight
                preserved_edges = torch.zeros(k_hop_subgraphs.shape, device=mask.device)
                preserved_edges[batch_id, edge_id] = 1

            masked_pred = self.model_to_explain(data, batch, edge_weight=edge_weight, preserved_edges=preserved_edges, drop_edges=drop_edges)
    

        assert not torch.any(torch.isnan(masked_pred))
        if not self.training:
            if save_explanation:
                return masked_pred, edge_weight
            return masked_pred
    ...
    def transform_mask(self, data, mask, k_hop_subgraphs=None, batch_id=None, edge_id=None):
        '''
        ...
        if self.eval_mask_type == 'hard_node_mask_top_ratio':
            ...
            return data, new_mask, drop_edges, new_batch_id, new_edge_id # the location of the mask changes
            ...
            edge_weight = torch.zeros(k_hop_subgraphs.shape, device=mask.device).fill_(float('-inf')) 
            ...
            ego_num_edges = k_hop_subgraphs.sum(dim=1)
            indices = torch.arange(edge_weight.size(1), device=edge_weight.device).expand_as(edge_weight)
            top_num_edges = (ego_num_edges*self.hard_edge_mask_top_ratio).to(torch.int32)
            ...
            return data, mask, drop_edges, new_batch_id, new_edge_id # the location of the mask changes

        elif self.eval_mask_type == 'soft_edge_mask':
            drop_edges = False
            return data, mask, drop_edges, batch_id, edge_id
