...
def prepare_data(split_data, batch, model):
    central_nodes = batch[:, :2].contiguous()
    label = batch[:, -1].to(torch.float64)
    rels = batch[:, -2]
    data = data_util.prepare_ego_data(split_data, central_nodes, model.hops)
...
def full_graph_prediction(data, batch, model):
    """
    Full Graph Prediction, get the original prediction and the node and rel embeddings from the NBFNet
    """
...
def relpgexplainer_train(train_data, batch, model, parallel_model, epoch, metric):
    data, batch, label = prepare_data(train_data, batch, model)
    _, node_embeds, R_embeds = full_graph_prediction(data, batch, model)
    pred, size_loss, mask_ent_loss, rw_loss = parallel_model(
        data, batch, node_embeds, R_embeds, epoch
    )
    pred = pred.squeeze()
    loss = F.binary_cross_entropy_with_logits(pred, label)
    metric.update(pred, label)
    return loss, size_loss, mask_ent_loss, rw_loss


def relpgexplainer_test(test_data, batch, model):
    data, batch, label = prepare_data(test_data, batch, model)
    _, node_embeds, R_embeds = full_graph_prediction(data, batch, model)
    pred, node_mask, edge_mask, num_edges, num_nodes = model(
        data, batch, node_embeds, R_embeds
    )
    return pred, label, node_mask, edge_mask, num_edges, num_nodes
...
def train_and_validate(
    ...
    train_loader = torch_data.DataLoader(train_edges, batch_size, sampler=sampler)
    ...
    for epoch in range(0, cfg.train.num_epoch):
        ...
        elif cfg.explainer["class"] == "RelPGExplainer":
            loss_dict = {
                "bce_loss": [],
                "size_loss": [],
                "mask_ent_loss": [],
        ...
        for batch in train_loader:  # batch: (batch_size, 3)
            ...
            if cfg.task.sample_neg_edges:
                ...
                neg_batch = negative_sampling(train_data, batch_size)
            ...
            if cfg.explainer["class"] == "PGExplainer":
                ...
                loss = relpgexplainer_train(
                    train_data, batch, model, parallel_model, epoch, metric
                )
                total_loss = 0
                for l, l_name in zip(
                    loss, ["bce_loss", "size_loss", "mask_ent_loss", "rw_loss"]
                ):
...
def test(
    ...
    for batch in test_loader:
        ...
        if cfg.explainer["class"] == "PGExplainer":
            ...
            pred, label, node_mask, edge_mask, num_edges, num_nodes = (
                relpgexplainer_test(test_data, batch, model)
            )
...
def run(cfg, args):
    ...
    train_and_validate(cfg, explainer, train_data, valid_data, logger, device)
