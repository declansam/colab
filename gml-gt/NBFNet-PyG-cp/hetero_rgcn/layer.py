...
class FastRGCNConv(RGCNConv):
    ...
    def forward(self, x: Union[OptTensor, Tuple[OptTensor, Tensor]],
                edge_index: Adj, edge_type: OptTensor = None, 
                edge_weight: OptTensor = None, edge_filter: OptTensor = None):
        ...
        out = self.propagate(edge_index, x=x_l, edge_type=edge_type, size=size,
                             edge_weight=edge_weight, edge_filter=edge_filter)
    ...
    def aggregate(self, inputs: Tensor, edge_type: Tensor, index: Tensor,
                  edge_weight: Tensor, edge_filter: Tensor,
                  dim_size: Optional[int] = None) -> Tensor:
        
        # edge weight
        if edge_weight is None:
            edge_weight = torch.ones_like(edge_type, dtype=inputs.dtype).unsqueeze(1)
        if edge_filter is None:
            edge_filter = torch.ones_like(edge_type, dtype=inputs.dtype)
        ...
        if self.aggr == 'mean':
            norm = torch.zeros((edge_type.size(0), self.num_relations), device=edge_type.device)
            norm[torch.arange(edge_type.size(0)), edge_type] = edge_filter
            # norm = one_hot(edge_type, self.num_relations, dtype=inputs.dtype) # one hot encoding of relation type for each edge (num_edges, num_rels)
            norm = scatter(norm, index, dim=0, dim_size=dim_size)[index] # for each tgt node, sum the # edges for each rel type. 
            norm = torch.gather(norm, 1, edge_type.view(-1, 1)) # for each tgt node, get the # edges for the corresponding rel type.
            norm = 1. / norm.clamp_(1.)
            inputs = norm * inputs # normalize each message

        return scatter(inputs, index, dim=self.node_dim, dim_size=dim_size) # aggr the messages for each node by summation